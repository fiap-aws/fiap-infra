...
data "aws_iam_policy_document" "fiap_devops_ecs_task_doc" {
  statement {
    actions = ["sts:AssumeRole"]
    effect  = "Allow"
...
resource "aws_iam_role" "fiap_devops_ecs_task_role" {
  name_prefix        = "fiap-devops-ecs-task-role"
  assume_role_policy = data.aws_iam_policy_document.fiap_devops_ecs_task_doc.json
}

resource "aws_iam_role" "fiap_devops_ecs_exec_role" {
  name_prefix        = "fiap-devops-ecs-exec-role"
  assume_role_policy = data.aws_iam_policy_document.fiap_devops_ecs_task_doc.json
}

resource "aws_iam_role_policy_attachment" "fiap_devops_ecs_exec_role_policy" {
  role       = aws_iam_role.fiap_devops_ecs_exec_role.name
